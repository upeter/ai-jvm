...
import dev.example.ColorPrinter.Color.*
        ...
        while (true) {
            ...
            if (userInput.isNotBlank()) {
                when (userInput) {
                    ...
                    "help" -> {
                        mapOf("text" to "Set to text chat",
                            "audio" to "Set to audio chat",
                            "restart" to  "Restart conversation (generates a new conversationId, so all previous history is removed)",
                            "exit" to "Exit the chat",
                            "help" to "Print this help message").forEach { (command, description) ->
                            cprintln(PURPLE, command)
                            cprintln(BLACK, "\t> $description")
                        }
                    }
                    "exit" -> {
